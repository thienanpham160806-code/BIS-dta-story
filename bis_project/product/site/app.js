...
function baseLayout(extra = {}) {
  const ink = css("--ink"), ink2 = css("--ink-2"), grid = css("--grid"), axis = css("--axis");
  return Object.assign({
    paper_bgcolor: "rgba(0,0,0,0)",
    plot_bgcolor: "rgba(0,0,0,0)",
    font: { family: 'system-ui, -apple-system, "Segoe UI", sans-serif', size: 11.5, color: ink2 },
    margin: { l: 52, r: 92, t: 42, b: 38 },
    hovermode: "x unified",
    hoverlabel: { bgcolor: css("--surface-1"), bordercolor: axis,
...
function phaseShapes() {
  ...
  return [
    band(p.hike[0], p.hike[1], css("--critical"), dark ? 0.13 : 0.07),
    band(p.ease[0], p.ease[1], css("--good"), dark ? 0.13 : 0.07),
...
function phaseAnnotations() {
  ...
  const tag = (x, y, text, color) => ({
    ...
    font: { size: 10, color: color || m }, xanchor: "left",
  });
  // Two rows: the 2026 tag sits above the others, because "Fed cắt 09/2024"
  // is wide enough to run under the risk band on the same line.
  return [
...
/** Right-hand direct labels — the relief for the low-contrast categorical slots.
 *  Labels are pushed apart vertically when two series end close together
 *  (e.g. Malaysia +0.2 vs Thailand −0.4 on the gap chart). */
function endLabels(traces) {
  ...
  return pts.map((p, i) => ({
    ...
    font: { size: 10.5, color: p.color, weight: 700 },
  }));
}

/** How many facet columns fit, given each needs ~minW px for its title. */
...
/** Render a chart. Plotly.react keeps the size captured when the plot was first
 *  drawn, which is stale whenever the container changed width while its tab was
 *  hidden — so always follow it with an explicit resize. */
...
function lineTrace(name, color, dates, vals, opts = {}) {
  return Object.assign({
    ...
    line: { color, width: opts.width || 2, dash: opts.dash },
...
function renderKPIs() {
  ...
  $("#kpis").innerHTML = S.countries.map((c) => {
    ...
    const txt = above > 4 ? "rủi ro cao" : above > 0.5 ? "còn căng" : "đã hồi phục";
    return `<div class="kpi" style="--k:${colorOf(c)}">
      <div class="name"><span style="width:9px;height:9px;border-radius:50%;background:${colorOf(c)};display:inline-block"></span>
        ${D.meta.names[c]}<span class="pill ${cls}" style="margin-left:auto">${txt}</span></div>
      ...
      <div class="row"><span>Đã gỡ phần vượt mốc</span><b>${r.repaid === null
        ? '<span style="font-weight:400;color:var(--muted)">chưa từng vượt mốc</span>' : r.repaid + "%"}</b></div>
      <div class="row"><span>Lãi suất hiện tại</span><b>${k.now.toFixed(2)}%</b>
        <span style="color:var(--muted)">đỉnh ${k.peak.toFixed(2)}%</span></div>
      <div class="row"><span>NPL ${x.last_year}</span><b>${x.npl_last.toFixed(2)}%</b>
        <span style="color:var(--muted)">${x.d_npl >= 0 ? "+" : ""}${x.d_npl} từ 2021</span></div>
    </div>`;
  }).join("");
}
...
function chartDSR() {
  ...
  $("#dsr-warn").innerHTML = only.length
    ? `⚠ BIS <b>chỉ</b> công bố DSR tổng PNFS cho ${only.map((c) => D.meta.names[c]).join(", ")} —
       không có breakdown Hộ gia đình / Doanh nghiệp. Không suy ra, không ước lượng.`
    : "";
...
function chartPolicy(el) {
  ...
  const us = lineTrace("Mỹ (Fed)", colorOf("US"), s.dates, s.cols.US, { unit: "%", width: 1.6, dash: "dash" });
  us.line.shape = "hv";
  traces.push(us);
...
function chartLagCorr() {
  ...
  // Taller when the facets stack, so each panel keeps a usable height.
...
function chartNPL(el) {
  ...
  // Year-end DSR only, so the quarterly series lines up with the annual NPL.
  ...
  cs.forEach((c, i) => {
    ...
    traces.push({ type: "scatter", mode: "lines", x: dx, y: dv.map((v) => v / base * 100),
      line: { color: colorOf(c), width: 2 }, xaxis: "x" + ax, yaxis: "y" + ax, showlegend: false,
    ...
    traces.push({ type: "scatter", mode: "lines+markers", x: n.dates,
      ...
      line: { color: css("--ink-2"), width: 1.6, dash: "dash" }, marker: { size: 4 },
...
function renderCoverage() {
  ...
  $("#cov").innerHTML =
    ...
    rows.map((r) => `<tr><td>${r.dataset}</td><td>${r.series}</td><td>${r.freq}</td>
      ...
      <td style="color:${r.missing_pct > 0 ? css("--critical") : "inherit"}">${r.missing_pct}%</td></tr>`).join("") +
    "</tbody>";
}
...
function renderStoryCharts() {
  // The story always shows all four countries: it is a fixed narrative, not a filter view.
...
function wire() {
  ...
  $("#chips").innerHTML = D.meta.order.map((c) =>
    `<button class="chip" data-c="${c}" aria-pressed="true"><span class="dot"></span>${D.meta.names[c]}</button>`
  ).join("");
  ...
  const show = (story) => {
    ...
    // Charts of a hidden tab cannot measure their container, so always re-render
    // the tab being shown — otherwise it keeps the layout from the last width.
  ...
  // A plain Plotly.Plots.resize would keep the old facet-column count, so a
  // desktop->phone resize would leave the 3-column lag grid overflowing.
  // Re-render instead, debounced.
  let rt = null;
...
fetch("data.json")
  ...
  .then((json) => {
    ...
    } catch (_) { /* private mode: stay on the default */ }
  ...
  .catch((err) => {
    document.body.insertAdjacentHTML("afterbegin",
      `<div class="wrap" style="padding:30px;color:#d03b3b">
